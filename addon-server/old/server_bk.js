...
<p>
<a href="https://pgcatalogs.duckdns.org/logout">Logout</a>
</p>
...
app.get("/catalogs", ensureLoggedIn, (req, res) => {
    ...
    html += "</ul>";
    html += "<p><a href='https://pgcatalogs.duckdns.org/dashboard'>back</a></p>"
    res.send(html);
});
...
app.get("/catalogs/:id/edit", ensureLoggedIn, (req, res) => {
    ...
    const genreCheckboxes = genreList.map(g => {
        const checked = config.genres.includes(g) ? "checked" : "";
        return `<input type="checkbox" name="genres" value="${g}" ${checked}> ${g.charAt(0).toUpperCase() + g.slice(1)}<br>`;
    }).join("");
    ...
    res.send(`
        ...
        <form method="POST" action="/catalogs/${catalog.id}/edit">
            ...
            <label>Catalog Name (no spaces)</label><br>
            <input name="name" value="${catalog.name}" required pattern="^[A-Za-z0-9_-]+$"><br><br>

            <label>Start Year (1900-2100)</label><br>
            <input type="number" name="start_year" min="1900" max="2100" value="${config.start_year}" required><br><br>

            <label>End Year (1900-2100)</label><br>
            <input type="number" name="end_year" min="1900" max="2100" value="${config.end_year}" required><br><br>

            <label>Pages to fetch (1-50)</label><br>
            <input type="number" name="pages" min="1" max="50" value="${config.pages}" required><br><br>

            <label>Language</label><br>
            <select name="language" required>
                ${["en-US","en-GB","fr-FR","es-ES","de-DE","it-IT","ja-JP","ko-KR","zh-CN"].map(lang =>
                    `<option value="${lang}" ${config.language === lang ? "selected" : ""}>${lang}</option>`
                ).join("")}
            </select><br><br>
            ...
            <select name="sort_order" required>
                ${[
                    ...
                    `<option value="${o}" ${config.sort_order === o ? "selected" : ""}>${o}</option>`
                ).join("")}
            </select><br><br>
            ...
            <label>Minimum critic score (0-100)</label><br>
            <input type="number" name="min_critic_score" min="0" max="100" value="${config.min_critic_score ?? ""}"><br><br>

            <button type="submit">Save Changes</button>
        </form>

        <p><a href="/catalogs">Back</a></p>
    `);
});
...
// Search Path
app.get("/addon/user/:uuid/catalog/:type/:catalogId/search=:query.json", (req, res) => {
    handleCatalog(req, res, decodeURIComponent(req.params.query));
});
// Lavenshtein 
...
// Shared Handler
function handleCatalog(req, res, pathSearch) {
    const userUuid = req.params.uuid;
    const type = req.params.type;
    const catalogId = req.params.catalogId.replace("catalog_", "");

    // ----------------------------
    // Resolve search + genre
    // ----------------------------
    let search =
        pathSearch ||
        req.query.search ||
        req.query["extra.search"] ||
        "";

    const genre =
        (req.query.genre || req.query["extra.genre"] || "")
            .toLowerCase()
            .trim();
    ...
    let data;
    try {
        data = JSON.parse(fs.readFileSync(catalog.generated_path, "utf8"));
    } catch (err) {
        console.error("Catalog read error:", err);
        return res.json({ metas: [] });
    }
    ...
    if (search) {
        ...
        filtered = filtered.filter(item => {
            const fields = [
                ...
                ...(Array.isArray(item.director) ? item.director : []),
                item.year
            ]
                .filter(Boolean)
    ...
    // ----------------------------
    // Shuffle only when browsing
    // ----------------------------
    const results = search || genre ? filtered : shuffle(filtered);
    ...
    const metas = results.map(item => ({
        ...
        year: item.year,
        links: [
            ...(Array.isArray(item.director) ? item.director : []).map(n => ({
                name: n,
                category: "Director",
                url: `stremio:///search?search=${encodeURIComponent(n)}`
            })),
            ...(Array.isArray(item.cast) ? item.cast : []).map(n => ({
                name: n,
                category: "Cast",
                url: `stremio:///search?search=${encodeURIComponent(n)}`
            }))
