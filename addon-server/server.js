...
app.get(
  /^\/addon\/user\/([^/]+)\/catalog\/([^/]+)\/([^/]+)\/(.+)\.json$/,
  (req, res) => {
    req.params = {
      uuid: req.params[0],
      type: req.params[1],
      catalogId: req.params[2],
      extra: req.params[3]
    };
    handleCatalog(req, res);
  }
...
function handleCatalog(req, res, pathSearch = "") {
    // -------------------------------------------------
    // Normalize extras passed as PATH segments
    // Example: /genre=Action/search=Batman.json
    // -------------------------------------------------
    ...
    // -------------------------------------------------
    // Resolve search + genre (query OR path)
    // -------------------------------------------------
    let search =
        pathSearch ||
        req.query.search ||
    ...
    try {
        data = JSON.parse(
            fs.readFileSync(catalog.generated_path, "utf8")
        );
    } catch (err) {
    ...
    // -------------------------------------------------
    // Shuffle only when browsing (no filters)
    // -------------------------------------------------
    const results =
        search || genre
            ? filtered
